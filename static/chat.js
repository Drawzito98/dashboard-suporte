...
(function () {
  ...
  function messageMarkup(message, userId) {
    ...
    return `<div data-chat-message-id="${esc(message.id)}" class="chat-message ${mine ? 'is-mine' : ''}">${reply}${image ? `<img class="chat-image" src="${esc(image)}" alt="Imagem enviada" loading="lazy">` : ''}${message.mensagem ? `<p>${esc(message.mensagem)}</p>` : ''}<div class="chat-message-meta"><time>${new Date(message.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</time>${mine ? `<span title="${message.lida_em ? 'Lida' : 'Enviada'}">${message.lida_em ? '✓✓' : '✓'}</span>` : ''}</div><div class="chat-message-actions"><button type="button" data-chat-action="reply" title="Responder">↩</button>${mine ? '<button type="button" data-chat-action="delete" title="Excluir">🗑</button>' : ''}</div>${reactionsMarkup(message, userId)}</div>`;
  }
  ...
  async function renderChatHome() {
    ...
    try {
      ...
      const users = (Array.isArray(payload) ? payload : payload.users || []).filter(item => item.app_metadata?.role !== 'admin' && item.app_metadata?.ativo !== false);
      ...
      const people = root.querySelector('#chatPeople'); people.innerHTML = users.length ? users.map(item => { const conversation = conversationByUser.get(item.id); const last = conversation && firstByConversation.get(conversation.id); const name = item.app_metadata?.csv_nome || item.user_metadata?.name || item.email; const preview = last ? (last.mensagem || '📷 Imagem') : 'Iniciar conversa'; const count = conversation ? unreadCounts.get(conversation.id) || 0 : 0; return `<button class="chat-person" data-id="${esc(item.id)}" data-label="${esc(name)}"><span class="chat-person-title"><strong>${esc(name)}</strong><span>${favorites.includes(item.id) ? '★' : ''}${count ? `<b class="chat-person-unread">${count}</b>` : ''}</span></span><small>${esc(preview.slice(0, 70))}${last ? ` · ${new Date(last.created_at).toLocaleDateString('pt-BR')}` : ''}</small></button>`; }).join('') : '<p class="chat-empty">Nenhum colaborador ativo.</p>';

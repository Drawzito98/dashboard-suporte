...
(function(){
  ...
  function context(){const g=activeFilters(),q=String(g.pesquisa||'').toLowerCase();return sourceRows().filter(valid).filter(r=>(!g.setor||g.setor==='all'||String(r.Setor)===String(g.setor))&&(!g.colaborador||g.colaborador==='all'||String(r.Atendente)===String(g.colaborador))&&(typeof g.correspondeNivel!=='function'||g.correspondeNivel(r))&&(!q||Object.values(r).some(v=>String(v??'').toLowerCase().includes(q))));}

...
(function () {
  ...
  function durationSeconds(value) { return typeof parseDurationToSeconds === 'function' ? parseDurationToSeconds(value) : null; }
  function fmtTime(value) { if(!Number.isFinite(value)) return '—'; const mins=Math.round(value/60), hours=Math.floor(mins/60), rest=mins%60; return hours ? `${hours}h ${rest}min` : `${mins}min`; }
  function median(values) { const list=values.filter(Number.isFinite).sort((a,b)=>a-b); if(!list.length)return null; const middle=Math.floor(list.length/2); return list.length%2?list[middle]:(list[middle-1]+list[middle])/2; }
  function averageTime(rows) { const values=rows.map(row=>durationSeconds(row.TMA)).filter(Number.isFinite); return values.length?values.reduce((a,b)=>a+b,0)/values.length:null; }
  function tmaBySector(context, currentRows) {
    const months=[...new Set(context.map(row=>String(row['Mês'])).filter(Boolean))].sort(), selectedMonths=[...new Set(currentRows.map(row=>String(row['Mês'])).filter(Boolean))].sort(), before=months.filter(value=>value<selectedMonths[0]).slice(-3);
    return [...new Set(currentRows.map(row=>String(row.Setor||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR')).map(name=>{
      const current=averageTime(currentRows.filter(row=>String(row.Setor||'').trim()===name)), baseValues=before.map(period=>averageTime(context.filter(row=>String(row.Setor||'').trim()===name&&String(row['Mês'])===period))).filter(Number.isFinite), baseline=median(baseValues);
      return {name,current,median:baseline,target:baseline,count:currentRows.filter(row=>String(row.Setor||'').trim()===name&&Number.isFinite(durationSeconds(row.TMA))).length,baseLabel:before.length?`${month(before[0])} a ${month(before.at(-1))}`:'sem base'};
  ...
  function scoreClass(value) { return value >= 4.7 ? 'score-excelente' : value >= 4.5 ? 'score-atencao' : 'score-critico'; }
  function transferClass(value) { return value <= .25 ? 'score-excelente' : value <= .30 ? 'score-atencao' : 'score-critico'; }
  ...
  function buildReport() {
    ...
    const tmaTargets=tmaBySector(context,currentRows); return {periodLabel,previousLabel,overall,overallHealth,sectors,people,tmaTargets,target,targetIsManual:Number(localStorage.getItem(TARGET_KEY))>0};
  }
  ...
  function renderOperationalHealth() {
    ...
    const {periodLabel,previousLabel,overall,overallHealth,sectors,people,tmaTargets,target,targetIsManual}=report;
    ...
    const tmaHtml=tmaTargets.length?`<section class="card so-section so-tma-section"><div class="so-section-heading"><div><span>REFERÊNCIA DE TMA</span><h3>TMA atual e meta por setor</h3><p>O atual respeita o filtro selecionado. A meta usa a mediana dos 3 meses anteriores.</p></div></div><div class="so-tma-grid">${tmaTargets.map(item=>`<article class="so-tma-card"><div><span>${esc(item.name)}</span><strong>${fmtTime(item.current)}</strong></div><p>Meta: ${fmtTime(item.target)} · Base: ${esc(item.baseLabel)}</p><small>${item.count} registro(s) no recorte atual</small></article>`).join('')}</div></section>`:'';
    host.innerHTML=`<div class="so-report" id="soPrintable"><section class="so-header card"><div><span class="page-eyebrow">Apresentação para gestão</span><h2>Saúde Operacional</h2><p>${esc(periodLabel)}${previousLabel?` comparado com ${esc(previousLabel)}`:''} · ${esc(scope.join(' · ')||'Toda a operação')}</p></div><button class="btn-secondary so-no-print so-print-btn" id="soPrintBtn">🖨️ Imprimir / PDF</button></section>
      ...
      <div class="so-kpis"><article><span>Pessoas no período</span><strong>${overall.people}</strong></article><article><span>Finalizados no período</span><strong>${fmt(overall.finished)}</strong><small>${overall.perPerson.toFixed(1).replace('.',',')} de média mensal por pessoa</small></article><article><span>Produtividade</span><strong>${pct(overall.productivity*100)}</strong></article><article><span>Qualidade</span><strong class="${overall.score?scoreClass(overall.score):''}">${overall.score?overall.score.toFixed(2):'—'}</strong></article><article><span>Transferências</span><strong>${pct(overall.transferRate*100)}</strong><small>menor é melhor</small></article></div>
      ${tmaHtml}<section class="card so-section"><div class="so-section-heading"><div><span>CONSOLIDADO</span><h3>Saúde dos setores</h3><p>${sectors.length} setor(es) · ${critical} crítico(s) · ${attention} em atenção</p></div></div><div class="so-sector-grid">${sectors.map(item=>`<article class="so-sector-card so-${item.health.key}"><header><div><span>SETOR</span><h4>${esc(item.name)}</h4></div><strong>${item.health.value}</strong></header>${statusBadge(item.health)}<div class="so-metrics"><span><small>Média mensal/pessoa</small><b>${item.current.perPerson.toFixed(1).replace('.',',')}</b></span><span><small>Produtividade</small><b>${pct(item.current.productivity*100)}</b></span><span><small>Qualidade</small><b class="${item.current.score?scoreClass(item.current.score):''}">${item.current.score?item.current.score.toFixed(2):'—'}</b></span><span><small>Transferências</small><b class="${transferClass(item.current.transferRate)}">${pct(item.current.transferRate*100)}</b></span></div><footer>${delta(item.health.evolution)}<small>Força: ${esc(item.health.strength)} · Atenção: ${esc(item.health.risk)}</small></footer></article>`).join('')}</div></section>
      <section class="card so-section"><div class="so-section-heading"><div><span>RESULTADOS INDIVIDUAIS</span><h3>Desempenho das pessoas</h3><p>Totais do período selecionado; nota calculada pela média mensal por pessoa.</p></div></div><div class="table-wrap"><table class="so-table"><thead><tr><th>#</th><th>Colaborador</th><th>Nível</th><th>Setor</th><th>Saúde</th><th>Finalizados no período</th><th>Média mensal</th><th>Produtividade</th><th>Qualidade</th><th>Transferências</th><th>Evolução</th></tr></thead><tbody>${people.map((item,index)=>`<tr><td>${index+1}</td><td><strong>${esc(item.name)}</strong></td><td>${esc(item.level||'—')}</td><td>${esc(item.sector||'—')}</td><td><b class="so-score so-${item.health.key}">${item.health.value}</b></td><td>${fmt(item.current.finished)}</td><td>${item.current.perPerson.toFixed(1).replace('.',',')}</td><td>${pct(item.current.productivity*100)}</td><td class="${item.current.score?scoreClass(item.current.score):''}">${item.current.score?item.current.score.toFixed(2):'—'}</td><td class="${transferClass(item.current.transferRate)}">${pct(item.current.transferRate*100)}</td><td>${delta(item.health.evolution)}</td></tr>`).join('')}</tbody></table></div></section>

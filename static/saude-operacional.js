...
(function () {
  ...
  function calculate(rows) {
    ...
    return { assumed,finished,transferred,people,score:scores.length?scores.reduce((a,b)=>a+b,0)/scores.length:0,productivity:assumed?finished/assumed:0,transferRate:assumed?transferred/assumed:0,perPerson:people?finished/people:0 };
  }
  function health(current, previous) {
    const evolution=previous&&previous.perPerson?((current.perPerson-previous.perPerson)/Math.abs(previous.perPerson))*100:null;
    const quality=clamp(current.score/4.7*100), productivity=clamp(current.productivity/.70*100), transfer=current.transferRate<=.25?100:clamp(100-(current.transferRate-.25)/.20*100), trend=evolution===null?70:clamp(70+evolution*2);
    const value=Math.round(quality*.35+productivity*.25+transfer*.25+trend*.15);
    const status=value>=80?'Saudável':value>=60?'Em atenção':'Crítico', key=value>=80?'healthy':value>=60?'attention':'critical';
    const factors=[{label:'qualidade',value:quality},{label:'produtividade',value:productivity},{label:'transferências',value:transfer},{label:'evolução',value:trend}].sort((a,b)=>a.value-b.value);
  ...
  function buildReport() {
    ...
    const currentMonth=selectedMonths.at(-1), index=contextMonths.indexOf(currentMonth), previousMonth=index>0?contextMonths[index-1]:null;
    const currentRows=selected.filter(row=>String(row['Mês'])===currentMonth), previousRows=previousMonth?context.filter(row=>String(row['Mês'])===previousMonth):[];
    const overall=calculate(currentRows), previousOverall=previousRows.length?calculate(previousRows):null, overallHealth=health(overall,previousOverall);
    const sectors=[...new Set(currentRows.map(row=>String(row.Setor||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR')).map(name=>{
      ...
      return {name,current,before,health:health(current,before)};
    ...
    const people=[...new Set(currentRows.map(row=>String(row.Atendente||'').trim()).filter(Boolean))].map(name=>{
      ...
      return {name,sector:[...new Set(rows.map(row=>row.Setor).filter(Boolean))].join(', '),level:levelOf(name),current,before,health:health(current,before)};
    }).sort((a,b)=>b.health.value-a.health.value);
    return {currentMonth,previousMonth,overall,overallHealth,sectors,people};
  }
  ...
  function renderOperationalHealth() {
    ...
    const {currentMonth,previousMonth,overall,overallHealth,sectors,people}=report;
    ...
    host.innerHTML=`<div class="so-report" id="soPrintable"><section class="so-header card"><div><span class="page-eyebrow">Apresentação para gestão</span><h2>Saúde Operacional</h2><p>${esc(month(currentMonth))}${previousMonth?` comparado com ${esc(month(previousMonth))}`:''} · ${esc(scope.join(' · ')||'Toda a operação')}</p></div><button class="btn-primary so-no-print" id="soPrintBtn">🖨️ Imprimir / PDF</button></section>
      <section class="so-hero so-${overallHealth.key}"><div><span>NOTA GERAL DE SAÚDE</span><strong>${overallHealth.value}<small>/100</small></strong>${statusBadge(overallHealth)}</div><p>${overallHealth.key==='healthy'?'A operação apresenta equilíbrio entre qualidade, produtividade, transferências e evolução.':overallHealth.key==='attention'?`A operação requer acompanhamento, principalmente em ${overallHealth.risk}.`:`A operação exige plano de ação prioritário, com maior impacto em ${overallHealth.risk}.`}</p></section>
      <div class="so-kpis"><article><span>Pessoas no período</span><strong>${overall.people}</strong></article><article><span>Finalizados</span><strong>${fmt(overall.finished)}</strong><small>${overall.perPerson.toFixed(1).replace('.',',')} por pessoa</small></article><article><span>Produtividade</span><strong>${pct(overall.productivity*100)}</strong></article><article><span>Qualidade</span><strong>${overall.score?overall.score.toFixed(2):'—'}</strong></article><article><span>Transferências</span><strong>${pct(overall.transferRate*100)}</strong><small>menor é melhor</small></article></div>
      <section class="card so-section"><div class="so-section-heading"><div><span>CONSOLIDADO</span><h3>Saúde dos setores</h3><p>${sectors.length} setor(es) · ${critical} crítico(s) · ${attention} em atenção</p></div></div><div class="so-sector-grid">${sectors.map(item=>`<article class="so-sector-card so-${item.health.key}"><header><div><span>SETOR</span><h4>${esc(item.name)}</h4></div><strong>${item.health.value}</strong></header>${statusBadge(item.health)}<div class="so-metrics"><span><small>Finalizados/pessoa</small><b>${item.current.perPerson.toFixed(1).replace('.',',')}</b></span><span><small>Produtividade</small><b>${pct(item.current.productivity*100)}</b></span><span><small>Qualidade</small><b>${item.current.score?item.current.score.toFixed(2):'—'}</b></span><span><small>Transferências</small><b>${pct(item.current.transferRate*100)}</b></span></div><footer>${delta(item.health.evolution)}<small>Força: ${esc(item.health.strength)} · Atenção: ${esc(item.health.risk)}</small></footer></article>`).join('')}</div></section>
      <section class="card so-section"><div class="so-section-heading"><div><span>RESULTADOS INDIVIDUAIS</span><h3>Desempenho das pessoas</h3><p>Ranking pela mesma metodologia de saúde aplicada aos setores.</p></div></div><div class="table-wrap"><table class="so-table"><thead><tr><th>#</th><th>Colaborador</th><th>Nível</th><th>Setor</th><th>Saúde</th><th>Finalizados</th><th>Produtividade</th><th>Qualidade</th><th>Transferências</th><th>Evolução</th></tr></thead><tbody>${people.map((item,index)=>`<tr><td>${index+1}</td><td><strong>${esc(item.name)}</strong></td><td>${esc(item.level||'—')}</td><td>${esc(item.sector||'—')}</td><td><b class="so-score so-${item.health.key}">${item.health.value}</b></td><td>${fmt(item.current.finished)}</td><td>${pct(item.current.productivity*100)}</td><td>${item.current.score?item.current.score.toFixed(2):'—'}</td><td>${pct(item.current.transferRate*100)}</td><td>${delta(item.health.evolution)}</td></tr>`).join('')}</tbody></table></div></section>
      <details class="card so-method"><summary>Como a nota de saúde é calculada?</summary><p>Qualidade 35% · produtividade 25% · transferências 25% · evolução de finalizados por pessoa 15%. Nota 80–100: saudável; 60–79: em atenção; abaixo de 60: crítico.</p></details></div>`;

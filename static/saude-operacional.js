...
(function () {
  ...
  function buildReport() {
    ...
    return {currentMonth,previousMonth,overall,overallHealth,sectors,people,target,targetIsManual:Number(localStorage.getItem(TARGET_KEY))>0};
  }
  ...
  function renderOperationalHealth() {
    ...
    const {currentMonth,previousMonth,overall,overallHealth,sectors,people,target,targetIsManual}=report;
    ...
    host.innerHTML=`<div class="so-report" id="soPrintable"><section class="so-header card"><div><span class="page-eyebrow">Apresentação para gestão</span><h2>Saúde Operacional</h2><p>${esc(month(currentMonth))}${previousMonth?` comparado com ${esc(month(previousMonth))}`:''} · ${esc(scope.join(' · ')||'Toda a operação')}</p></div><button class="btn-primary so-no-print" id="soPrintBtn">🖨️ Imprimir / PDF</button></section>
      ...
      <details class="card so-method"><summary>Como a nota de saúde é calculada?</summary><p>Qualidade 45% · quantidade finalizada por pessoa 35% · transferências 15% · evolução mensal 5%. Nota 80–100: saudável; 60–79: em atenção; abaixo de 60: crítico.</p><div class="so-target so-no-print"><label for="soTargetInput">Meta mensal de finalizados por pessoa</label><input id="soTargetInput" type="number" min="1" step="1" value="${Math.round(target)}"><button class="btn-primary" id="soSaveTarget">Salvar meta</button><button class="btn-secondary" id="soAutoTarget">Usar média automática</button><small>${targetIsManual?'Meta personalizada ativa.':'Referência automática: média atual da operação.'}</small></div></details></div>`;

...
(function () {
  ...
  function tmaBySector(context, currentRows) {
    return [...new Set(currentRows.map(row=>String(row.Setor||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR')).map(name=>{
      const values=context.filter(row=>String(row.Setor||'').trim()===name).map(row=>durationSeconds(row.TMA)).filter(Number.isFinite);
      const current=context.filter(row=>String(row.Setor||'').trim()===name && currentRows.some(item=>String(item['Mês'])===String(row['Mês']))).map(row=>durationSeconds(row.TMA)).filter(Number.isFinite);
      const historical=median(values); if(historical===null)return {name,median:null,target:null,current:null,count:0};
      return {name,median:historical,target:Math.max(60,historical*.95),current:current.length?current.reduce((a,b)=>a+b,0)/current.length:null,count:values.length};
  ...
  function renderOperationalHealth() {
    ...
    const tmaHtml=tmaTargets.length?`<section class="card so-section so-tma-section"><div class="so-section-heading"><div><span>METAS SUGERIDAS</span><h3>TMA ideal por setor</h3><p>Mediana histórica com melhoria inicial de 5%, sem separar por nível.</p></div></div><div class="so-tma-grid">${tmaTargets.map(item=>`<article class="so-tma-card"><div><span>${esc(item.name)}</span><strong>${fmtTime(item.target)}</strong></div><p>Histórico: ${fmtTime(item.median)} · Atual: ${fmtTime(item.current)}</p><small>${item.count} registro(s) de TMA analisado(s)</small></article>`).join('')}</div></section>`:'';
